<?php 
session_start(); 
if (empty($_SESSION['LMNS-USER_USUARIOC'])){      
...
$agentes=0;
$consulta="SELECT count(*) FROM agentes a inner join usuarios u on u.usuario=a.usuario WHERE a.unidad_negocio='".$_SESSION['LMNS_unidad']."' and u.estado='Activo' "; 
...
$consulta="SELECT count(*) FROM actividadclientes WHERE codcliente='".$_SESSION['LMNS-USER_CLIENTE']."' "; 
}else{
$consulta="SELECT count(*) FROM actividad WHERE    proyecto='Especial Enel'   "; 	
}
 $datos=mysqli_query($mysqli,$consulta);      
if($row=mysqli_fetch_row($datos)){ 
$actividades=$row[0];
...
fecha>= DATE_ADD(CURDATE(),INTERVAL -17 DAY)  AND codcliente='".$_SESSION['LMNS-USER_CLIENTE']."' GROUP BY fecha"; 
}else{

  $consulta="SELECT fecha,SUM(cantidad),SUM(efectivos),SUM(noefectivos) FROM actividad WHERE 
fecha>= DATE_ADD(CURDATE(),INTERVAL -17 DAY)  AND proyecto='Especial Enel'  GROUP BY fecha"; 
}
            ...
            <div class="col-md-4 content-top-2 card widget" style="width: 51% !Important">
        		<div class="r3_counter_box">
                    ...
                    <i class="pull-left fa fa-star" style="font-size: 50PX"></i>
                     ¡Bienvenido  a Lumens!
					<br><b><?php echo  $_SESSION['LMNS-USER_NOMBREC'];?></b></h2>
