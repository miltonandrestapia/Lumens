<?php
	...
	$filename = "Busqueda".$_GET["BusquedaParametro"].".xls"; 
...
$completa="";
if($_GET["BusquedaTipo"]==""){
  $completa="  (p.resultado1='".mysqli_real_escape_string($mysqli,$_GET["BusquedaParametro"])."' 
  or p.resultado2='".mysqli_real_escape_string($mysqli,$_GET["BusquedaParametro"])."') ";
}else{
 $completa=" p.".$_GET["BusquedaTipo"]."='".mysqli_real_escape_string($mysqli,$_GET["BusquedaParametro"])."'";
}

if($_GET["BusquedaCodcl"]=="N/A"){
  $completa.=" AND p.codcliente='".mysqli_real_escape_string($mysqli,$_GET["BusquedaCodcl"])."' ";
}

   $consulta="SELECT  ac.cons, ac.fecha, ac.tipo, ac.soporte, ac.usuariocarga, ac.estado, ac.fechacreacion,ac.fechaanulado, ac.observaciones,p.destinatario,p.direccion,c.nombre,d.nombre,p.cliente,p.referencia1,p.referencia2,p.referencia3,p.referencia4,p.referencia5,p.ruta,p.ordenamiento,a.nombre,p.estado,p.fecharealizado,p.respuesta,p.detallerespuesta,p.resultado1,p.resultado2,p.platitud,p.plongitud,p.soporte,p.observaciones,p.cons as pcons
     FROM ((((principal p INNER JOIN actividad ac ON ac.cons=p.codactividad) INNER JOIN ciudades c ON c.cons=p.codciudad)INNER JOIN departamentos d ON d.cons=p.coddepartamento)INNER JOIN agentes a ON a.usuario=p.codagente)
WHERE ". $completa." AND ac.fecha>='".mysqli_real_escape_string($mysqli,$_GET["BusquedaDesde"])."' 
      and ac.fecha<='".mysqli_real_escape_string($mysqli,$_GET["BusquedaHasta"])."' AND 
     p.codempresa='".mysqli_real_escape_string($mysqli,$_GET["BusquedaCode"])."' ORDER BY ac.fecha DESC,p.cons DESC   "; 
